...
def parse_args():
    ...
    parser.add_argument('-m', '--model', type=str,default='MobileNets')
...
def main(args):
    ...
    for picture in pictures:
        try:
            img = Image.open(os.path.join(outputPath, picture))
            img_resized = img.resize((224, 224))  # Resize the image to (224, 224)
            img_ndarray = np.asarray(img_resized, dtype='float64') / 255
    ...
    for modelName in modelsList:
        ...
        fakeSum += fakeCount
        rawSum += rawCount
        print("fakeCount=", fakeCount, "rawCount=", rawCount)

    if fakeSum > rawSum:
        print("The video is fake.")
    else:
        print("The video is not fake.")
